...
private:
    ...
    int nb_groups_detected;
    geometry_msgs::Point groups_detected[1000];// to store the middle of each moving person

    int groups_of_persons[1000];
    int nb_of_persons_in_group[1000];
...
void detect_group_of_persons(){
  ROS_INFO("detecting groups");
  nb_groups_detected = 1;
  groups_of_persons[0] = 0;
  nb_of_persons_in_group[0] = 1;
  for(int loop = 1; loop < nb_moving_persons_detected; loop++){
    float min_distance = 1000;
    int min_person_id = -1;
    for(int loop2 = 0; loop2 < loop; loop2++){
        float distance = distancePoints(moving_persons_detected[loop], moving_persons_detected[loop2]);
        if(distance < min_distance && distance < group_threshold){
    ...
    if(min_person_id != -1){
      ...
      groups_of_persons[loop] = groups_of_persons[min_person_id];
      nb_of_persons_in_group[groups_of_persons[min_person_id]]++;

    }else{
      //create new group
      groups_of_persons[loop] = nb_groups_detected++;
      nb_of_persons_in_group[nb_groups_detected-1]++;
    }
  }



  float sum_of_x[nb_groups_detected], sum_of_y[nb_groups_detected];

  for(int i = 0; nb_groups_detected; i++){
      sum_of_x[i] = 0;
      sum_of_y[i] = 0;
  }
  for(int j = 0; j<nb_moving_persons_detected; j++){
      sum_of_x[groups_of_persons[j]] += moving_persons_detected[j].x;
      sum_of_y[groups_of_persons[j]] += moving_persons_detected[j].y;
  }
  for(int i = 0; nb_groups_detected; i++){
      groups_detected[i].x = sum_of_x[i]/nb_of_persons_in_group[i];
      groups_detected[i].y = sum_of_y[i]/nb_of_persons_in_group[i];
  }


  ROS_INFO("%d moving groups have been detected.\n", nb_groups_detected);

  for(int i = 0; nb_groups_detected; i++){
    display[nb_pts].x = groups_detected[i].x;
    display[nb_pts].y = groups_detected[i].y;
    display[nb_pts].z = groups_detected[i].z;
